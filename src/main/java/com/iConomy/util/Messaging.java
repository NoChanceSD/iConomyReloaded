...
public class Messaging {
	 ...
	 * @param arguments
	 *            The list of arguments, multiple arguments are seperated by commas for a single
	 *            point.
	 * @param points
	 ...
	 *         (points)
	 */
	public static String argument(String original, final Object[] arguments, final Object[] points) {
		for (int i = 0; i < arguments.length; i++) {
			if (String.valueOf(arguments[i]).contains(",")) {
				for (final String arg : String.valueOf(arguments[i]).split(",")) {
	...
	/**
	 * Parses the original string against color specific codes. This one converts &[code] to §[code] <br />
	 * <br />
	 * Example: <blockquote>
	 * 
	 * <pre>
	...
	public static String colorize(String original) {
		...
		return original.replace("<black>", "\u00A70").replace("<navy>", "\u00A71").replace("<green>", "\u00A72").replace("<teal>", "\u00A73")
				.replace("<red>", "\u00A74").replace("<purple>", "\u00A75").replace("<gold>", "\u00A76").replace("<silver>", "\u00A77")
				.replace("<gray>", "\u00A78").replace("<blue>", "\u00A79").replace("<lime>", "\u00A7a").replace("<aqua>", "\u00A7b")
	 ...
	 */
	public static String bracketize(final String message) {
		return "[" + message + "]";
	}
