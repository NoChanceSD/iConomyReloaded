...
public class Misc {
	...
	public static String formatted(final String amount, final List<String> maj, final List<String> min) {
		...
		if (Constants.FormatMinor) {
			...
			final int plural = plural(Double.valueOf(famount));
			final String currency = maj.get(plural);
	 ...
	 */
	public static Player playerMatch(final String name) {
		final Collection<? extends Player> online = iConomy.getBukkitServer().getOnlinePlayers();
